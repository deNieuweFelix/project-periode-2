//zet de elementen in variabelen zodat ze makkelijk te gebruiken zijn
...
//maakt een class, wat niet de beste manier was nu ik terug kijk, maar het werkt en dat is het belangrijkste (:
class game{
...
//functie om van game te veranderen, volgensmij is dit niet per se nodig, maar het maakt het gebruik een stuk makkelijker
function nextGame(prev){
    let pegiImage = undefined;

    //increment of decrement currentgame gebaseerd op de ingedrukte knop!!!
    let starsDisplay = [];
    if(prev == true){
        currentGame--;
    }else{
    ...
    //zorgt ervoor dat je niet verder kan gaan dan het aantal games
    ...
    //laat het aantal sterren zien met de ster emoji
    for(i = 0; i < felix_js_data[currentGame].sterren; i++){
        starsDisplay.push("⭐");
    }
    let multiplayer = "";

    //kijkt wat de pegi is, en maakt dan de afbeelding deze pegi.
    ...
    //kijkt wat de current game is 0 of 1
    switch(currentGame){
        case 0:
            //gebruik innerhtml en src om het op de pagina te laten zien
            ...
            //kijkt of de game multiplayer heeft
            ...
            break;
        case 1:
            //hetzelfde als hierboven in principe
...
//ik ben vergeten wat dit doet, maar het ziet er belangrijk uit
for(i = 0; i <= 2; i++){
    nextGame();
}